...
function addToShoppingCart(productName) {
  ...
  shoppingCart.push(p)
  updateConceptPaneWithProduct(p, true)
}
function removeFromShoppingCart(productName) {
  const i = shoppingCart.findIndex(x => x.product == productName)
  const p = shoppingCart.splice(i, 1)
  updateConceptPaneWithProduct(p, false)
}
// const [addToShoppingCart, removeFromShoppingCart] = (() => {
//   let idx = 0
//   function add() {
//     console.log('add to shopping cart with idx', idx)
...
//   }
//   function remove() {
//     console.log('remove from shopping cart with idx', idx)
//     if (idx <= 0) return;
//     const p = shoppingCart.pop()
//     idx--;
//     updateConceptPaneWithProduct(p, false)
//   }
//   return [add, remove]
...
function fetchPageData() {
  ...
  return fetch(url)
    ...
    .then(j => {
      ...
      [...answers, ...collapsedAnswers].forEach(({propositions}) => {
        // console.log(propositions)
        propositions.forEach(({concept, subconcept}) => {
          // console.log(concept, subconcept)
          if (!conceptBySubconcept[concept]) {
            conceptBySubconcept[concept] = new Set()
          }
          conceptBySubconcept[concept].add(subconcept)
        })
      });

      for (const con in conceptBySubconcept) {
        const subConArray = Array.from(conceptBySubconcept[con]);
        const othersIdx = subConArray.indexOf('Others')
        if (othersIdx > 0) {
          subConArray.push(subConArray.splice(othersIdx, 1)[0])
        }
        conceptBySubconcept[con] = subConArray
      }
      console.log(conceptBySubconcept)
...
function getSelectedConcept() {
  return document.querySelector('#concept-list-container .badge[selected]').getAttribute('concept-name')
}

function getSelectedSubconcept() {
  return document.querySelector('#subconcept-list-container .badge[selected]').getAttribute('concept-name')
}
...
function onModalPropositionClicked(el, checkboxEl, isClickingCheckbox) {
  const data = getElData(el)
...
function onModalProductClicked(el) {
  const containerEl = el.closest('.product-item')
  let checkboxEl = el
  if (el.tagName.toUpperCase() != 'INPUT') { // If not clicking the checkbox but the card
    checkboxEl = containerEl.querySelector('input[type="checkbox"]')
    checkboxEl.checked = !checkboxEl.checked
  }

  const {checked} = checkboxEl
  const productName = containerEl.querySelector('.card-body').textContent.trim()
  if (checked) {
...
function createBadge(textContent, isSub) {
  const badge = document.createElement('span')
  badge.textContent = textContent
  badge.classList.add('badge', 'bg-secondary', 'me-1', isSub ? 'subconcept-badge' : 'concept-badge')
  // badge.title = 'Add some marked propositions to generate the mind map.'
  badge.setAttribute('previous-color', 'bg-secondary')
  badge.setAttribute('concept-name', textContent)
  badge.setAttribute('mindmap-concept', textContent)
  return badge
}

function createProductListItem(product, isFullCart) {
  const {product: name, src: filename} = product
  const src = `hackathon/images/${filename}`
  const item = document.createElement('li')
  if (isFullCart) {
    item.classList.add('list-group-item', 'product-item', 'd-flex', 'align-items-center')
    item.innerHTML = `<img src=${src}>
    <div class="flex-fill">${name}</div>
    <input type="checkbox" class="form-check-input m-0">
    `
  } else {
    item.classList.add('list-group-item', 'product-item')
    item.innerHTML = `<img src=${src}>
    <div>${name}</div>
    `
  }
...
function initConceptPane(answers) {
  ...
  conceptListContainer.append(...Array.from(conceptSet, c=>createBadge(c)))
}
...
function initSubConceptModal() {
  ...
  // 当打开时（加载modal内容）
  subConceptModalEl.addEventListener('show.bs.modal', () => {
    const subconcept = getSelectedSubconcept()
    const selectedProducts = shoppingCart.map(p => p.product)
    ...
    const propElList = answers
      .reduce((acc, cur, ansIdx) => [
        ...
        ...cur.propositions.map((p, propIdx) => {
          ...
          el.innerHTML = `<div>
            <span class="proposition" data-proposition="${propIdx}" data-answer="${ansIdx}">${p.content}</span><input type="checkbox" class="form-check-input">
          </div>
          <div class="w-100 d-flex overflow-auto product-list">
            ${
              (p.products || []).map(prodIdx => {
                const {product: name, src: filename} = products[prodIdx]
                const src = `hackathon/images/${filename}`
                const checked = selectedProducts.indexOf(name) >= 0
                return `<div class="card me-2 product-item">
...
function initEditModal() {
  ...
  const titleTypeEl = editModalEl.querySelector('#editModalLabel>span');
  /** @type HTMLTextAreaElement */
  const textInputEl = editModalEl.querySelector('#edit-input');
  /** @type HTMLFormElement */
  const editForm = Array.prototype.find.call(document.forms, f => (f.id == 'editModalForm'))

  editModal = new bootstrap.Modal(editModalEl)
  editModalEl.addEventListener('show.bs.modal', e => {
    const button = e.relatedTarget.closest('button')
    const {type} = button.dataset
    const oldContent = lastPopoverReference.closest('li').querySelector('.content').textContent;
    titleTypeEl.textContent = type
    textInputEl.value = oldContent
    textInputEl.rows = type == 'proposition' ? 3 : 1
  })

  textInputEl.addEventListener('keydown', e => {
    if (e.code == 'Enter') {
      e.preventDefault()
      console.log('No Enter in this textarea!')
...
document.addEventListener('click', (e) => {
  ...
  } else if (e.target.matches('.subconcept-badge')){
    onSubconceptBadgeClick(e.target)
  } else if (e.target.matches('#btn-show-subconcept-modal')) {
    subConceptModal.show(e.target)
  } else if (e.target.matches('#subConceptModal .proposition')) {
    onModalPropositionClicked(e.target, e.target.nextElementSibling, false)
  } else if (e.target.matches('#subConceptModal .proposition~input[type="checkbox"]')) {
    onModalPropositionClicked(e.target.previousSibling, e.target, true)
  } else if (e.target.matches('#subConceptModal .product-item, #subConceptModal .product-item *')) {
    onModalProductClicked(e.target)
...
function onConceptBadgeClick(el) {
  ...
  // 切换选中节点
  const conceptBadgeEls = document.querySelectorAll(`#concept-list-container .badge`)
  ...
  // 显示subconcept
  const concept = el.getAttribute('concept-name')
  const subBadges = conceptBySubconcept[concept].map(sub => createBadge(sub, true))
  document.getElementById('subconcept-list-container').replaceChildren(...subBadges)

  // 清空子购物车
  const subcartContainer = document.getElementById('mindmap-container')
  subcartContainer.classList.add('d-none')
  subcartContainer.querySelector('.list-group').innerHTML = ''
}

function onSubconceptBadgeClick(el) {
  const isCurrentlySelected = el.hasAttribute('selected')
  if (isCurrentlySelected) return
  // 切换选中节点
  const conceptBadgeEls = document.querySelectorAll(`#subconcept-list-container .badge`)
  conceptBadgeEls.forEach(p => {
    p.removeAttribute('selected')
  })
  el.setAttribute('selected', '')
  ...
  // 切换显示子购物车
  const subcartContainer = document.getElementById('mindmap-container')
  subcartContainer.classList.remove('d-none')
  updateConceptPaneWithProduct()
}

function updateConceptPaneWithProduct(p, isAdd) {
  // if no arg (p=null), redraw entire concept pane shopping cart list
  // if provided p, add or remove based on the boolean isAdd
  const subcartList = document.querySelector('#mindmap-container .list-group')
  const fullcartList = document.querySelector('#full-cart-container .list-group')
  const concept = getSelectedConcept()
  const subconcept = getSelectedSubconcept()
  // 子购物车
  // if (!p) {
    const subProductEls = shoppingCart
      .filter(p => p.concept == concept && p.subconcept == subconcept)
      .map(p => createProductListItem(p))
    subcartList.replaceChildren(...subProductEls)
  // } else if (concept == p.concept && subconcept == p.subconcept) {
  //   console.log('Updating aspect pane shopping cart')
  //   if (isAdd) {
  //     subcartList.append(createProductListItem(p))
  //   } else {
  //     subcartList.removeChild(subcartList.lastChild)
  //   }
  // }
  // 总购物车
  // if (!p) {
    const productEls = shoppingCart.map(p => createProductListItem(p, true))
    fullcartList.replaceChildren(...productEls)
  // } else {
  //   if (isAdd) {
  //     fullcartList.append(createProductListItem(p, true))
  //   } else {
  //     fullcartList.removeChild(fullcartList.lastChild)
  //   }
  // }
}
